...
async def on_member_join(member):
    log = discord.utils.get(member.guild.text_channels, name=CANAL_LIBERACAO)
    if log:
        embed = discord.Embed(title="🐲 NOVO MEMBRO!", description=f"{member.mention} quer entrar no bando!", color=0x00FF7F)
        await log.send(embed=embed, view=AprovarMembroView(member.id))
    ...
    if jogo_em_andamento["resposta"] and message.channel.name == CANAL_GERAL:
        ...
        if tipo == "roleta" and msg == "roleta":
            ...
            if res == "1000":
                ...
                pontuacao_monstrinho[uid] = max(0, pontuacao_monstrinho.get(uid, 0) - 200)
                await message.reply("📉 A roleta parou no vermelho! Perdeu 200 coins.")
            elif res == "jogo":
                await message.reply("🎲 A roleta te deu um novo jogo! Disparando...")
                await disparar_pergunta(message.guild)
            else:
                valor = int(res)
                pontuacao_monstrinho[uid] = pontuacao_monstrinho.get(uid, 0) + valor
                await message.reply(f"🎡 Ganhou {valor} coins!")
    ...
    if not any(r.name in CARGOS_IMUNES_NOMES for r in message.author.roles) and message.channel.name != CANAL_DESABAFOS:
        if any(p in message.content.lower() for p in PALAVRAS_PROIBIDAS):
            ...
            if avisos_usuarios[message.author.id] >= 4:
                ...
                await message.channel.send(f"🚨 {message.author.mention} foi castigado por 24h!", view=LiberarCastigoView(message.author.id))
